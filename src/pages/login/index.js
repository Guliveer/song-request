...
export default function Login() {
  ...
  useEffect(() => {
    if (isLoggedIn && !["/reset-password", "/login"].includes(window.location.pathname)) {
      router.push("/");
    }
  }, [isLoggedIn, router]);
  ...
  async function handleLogin(e) {
    ...
    if (aalData.currentLevel === "aal1" && aalData.nextLevel === "aal2") {
      ...
      const totpFactor = factorsData?.all?.find((f) => f.status === "verified" && f.factor_type === "totp");
  ...
  async function handleForgotPassword() {
    ...
    const redirectTo = process.env.NEXT_PUBLIC_REDIRECT_URL || "./reset-password";
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-8 items-center justify-center h-[90vh] px-4">
        ...
        <form onSubmit={handleLogin} className="flex flex-col gap-4 w-fit max-w-sm">
          <Input type="email" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} required />
          <Input type="password" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} required />

          <HCaptcha ref={captcha} sitekey="b224d136-6a4c-407a-8d9c-01c2221a2dea" theme="dark" onVerify={(token) => setCaptchaToken(token)} />
          ...
          <Button type="submit" disabled={isSubmitting || !email || !password || !captchaToken} className="w-full">
            {isSubmitting ? (
      ...
      <Dialog open={showForgotPassword} onOpenChange={setShowForgotPassword}>
        <DialogContent className="sm:max-w-md">
          ...
          <p className="text-sm text-muted-foreground mb-2">Enter your email address. You will receive a link to reset your password.</p>
          ...
          <HCaptcha ref={forgotCaptcha} sitekey="b224d136-6a4c-407a-8d9c-01c2221a2dea" theme="dark" onVerify={(token) => setForgotCaptchaToken(token)} />
          <DialogFooter>
            <Button
              type="button"
              variant="ghost"
            ...
            <Button onClick={handleForgotPassword} disabled={isSending || !forgotEmail || !forgotCaptchaToken}>
      ...
      <Dialog open={showMfaDialog} onOpenChange={setShowMfaDialog}>
        <DialogContent className="sm:max-w-md">
          ...
          <Input placeholder="6-digit code" value={mfaCode} onChange={(e) => setMfaCode(e.target.value)} inputMode="numeric" maxLength={6} className="tracking-widest text-center text-lg font-semibold" />
