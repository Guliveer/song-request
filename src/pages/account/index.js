...
import Followers from "@/components/userPanel/Followers";
import Providers from "@/components/userPanel/Providers";
import Playlists from "@/components/userPanel/Playlists";
import { Container } from "shadcn/container";
import { Typography } from "shadcn/typography";
import { Avatar, AvatarFallback, AvatarImage } from "shadcn/avatar";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "shadcn/tabs";
import { Separator } from "shadcn/separator";
import { Link as LinkIcon, Music as PlaylistsIcon, User as PersonIcon, Users as PeopleIcon } from "lucide-react";
...
export default function UserPanel() {
    ...
    useEffect(() => {
        const checkUser = async () => {
            ...
            const {
                data,
                error
            } = await supabase.from("users").select("username, color, emoji").eq("id", user.id).single();
    ...
    const getFollowersCount = async () => {
        if (!userId) return;
        const {count, error} = await supabase.from("users").select("id", {
            count: "exact",
            head: true
    ...
    return (
        <>
            ...
            <Container className="mt-6 mb-10">
                {/* Sekcja profilowa */}
                <div
                    className="max-w-6xl mx-auto mt-12 mb-8 px-2 md:px-8 py-4 md:py-10 rounded-xl bg-card flex flex-col items-center">
                    <Avatar className="w-24 h-24 mb-4 shadow-lg">
                        <AvatarImage src={avatarUrl} alt="Avatar"/>
                        <AvatarFallback
                            className="text-2xl font-bold">{userProfile?.username?.[0]?.toUpperCase() || "U"}</AvatarFallback>
                    </Avatar>
