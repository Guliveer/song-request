...
import { getUserInfo, getSongData, getCurrentUser, removeUserVote, updateUserVote, playSound, genUserAvatar } from "@/lib/actions";
...
import { ExternalLink, Music, CirclePlay, StopCircle, User, ChevronUp as VoteUp, ChevronDown as VoteDown, Trash2, Ban, RotateCw, CalendarClock as DateAddedIcon } from "lucide-react";
...
function SongCard({ id, currentlyPreviewingSongId, setCurrentlyPreviewingSongId }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (currentUser) {
        ...
        const { data: banData, error: banError } = await supabase.from("users").select("ban_status").eq("id", currentUser.id).single();

        if (!banError && banData?.ban_status > 0) {
          setIsBanned(true);
        }

        const { data: voteData, error: voteError } = await supabase.from("votes").select("vote").eq("song_id", id).eq("user_id", currentUser.id).single();
        ...
        const { data: userData, error: userError } = await supabase.from("users").select("followed_users").eq("id", currentUser.id).single();
        ...
        if (!userError && userData.followed_users) {
          ...
          if (userData.followed_users.length > 0) {
            const { data: followedVotes, error: followedVotesError } = await supabase.from("votes").select("user_id, vote").eq("song_id", id).in("user_id", userData.followed_users);

            if (!followedVotesError && followedVotes) {
              const votesWithUsernames = await Promise.all(
                followedVotes.map(async (vote) => {
  ...
  const handleVote = useCallback(
      ...
      try {
        if (userVote === newVoteValue) {
          ...
          if (
            ...
            const { data: existingNotifications } = await supabase.from("notifications").select("id").eq("user_id", songData.rawUserId).eq("sender_id", user.id).eq("type", "song_like").eq("link", `/song/${id}`);

            if (!existingNotifications || existingNotifications.length === 0) {
              let username = user?.username;
              if (!username) {
                const { data: userInfo } = await supabase.from("users").select("username").eq("id", user.id).single();
    ...
    [user, userVote, id, fetchData, songData]
  );
  ...
  const handleResetVotes = async () => {
    const { error } = await supabase.from("votes").delete().eq("song_id", id);
  ...
  const handleBanAndDelete = async () => {
    const { error: insertError } = await supabase.from("banned_url").insert([{ url: songData.url }]);
  ...
  const handleFollow = async () => {
    ...
    try {
      const { data: userData, error: userError } = await supabase.from("users").select("followed_users").eq("id", user.id).single();
      ...
      const { error: updateError } = await supabase.from("users").update({ followed_users: updatedFollowed }).eq("id", user.id);
  ...
  return (
    <Card onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} className={"px-2 transition-all duration-200 w-fit h-min cursor-default border rounded-2xl relative overflow-hidden hover:shadow-xl"}>
      <CardContent className={"py-2"}>
        ...
        <div className="flex items-center justify-between">
          ...
          <TooltipProvider>
            <Tooltip variant={"outline"}>
              ...
              <TooltipContent side="bottom">Open song page</TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
        ...
        <div className="flex gap-3 items-center my-4">
            ...
            {hovered && (
              <div className="absolute inset-0 bg-black/30 flex items-center justify-center rounded-md transition-opacity">
                <Button
                  ...
                  {isSpotify ? spotifyConnected && showPlayer ? <StopCircle className="w-4 h-4" /> : <FontAwesomeIcon icon="fa-brands fa-spotify" className={"w-4 h-4"} /> : currentlyPreviewingSongId === id ? <StopCircle className="w-4 h-4" /> : <CirclePlay className="w-4 h-4" />}
                  {spotifyConnected && showPlayer && trackId && <SpotifyPlayer trackId={trackId} />}
                </Button>
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col items-center gap-1 py-1 min-w-[48px] w-fit">
            <Button size="icon" variant="ghost" className={`w-8 h-8 hover:bg-white/10`} onClick={() => handleVote(1)} disabled={isBanned}>
          ...
          <div className="flex items-center justify-end mt-4 gap-0.5">
            ...
            {followedUsersVotes.length > 3 && (
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div className="w-5 h-auto aspect-square flex items-center justify-center rounded-full bg-muted text-xs cursor-default">+{followedUsersVotes.length - 3}</div>
                  </TooltipTrigger>
                  <TooltipContent side="bottom">{`${followedUsersVotes.length - 3} more voted`}</TooltipContent>
                </Tooltip>
              </TooltipProvider>
            )}
          </div>
        ...
        {/* Error Display */}
        {error && <div className="mt-4 text-sm text-destructive bg-destructive/10 border border-destructive/20 rounded-md p-2">{error}</div>}
      </CardContent>
    </Card>
  );
}
