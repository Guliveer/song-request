...
import { getPlaylistData, getPlaylistModerators, getPlaylistMembers, getPlaylistBannedUsers, banPlaylistUser, unbanPlaylistUser, leavePlaylist, getCurrentUser } from "@/lib/actions";
...
import { Star as HostIcon, Shield as ModIcon, Ban as BanIcon, X as KickIcon, Undo as UnbanIcon } from "lucide-react";
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-4 gap-4">
        <FormField label="Search Users" variant="outlined" size="small" value={searchQuery} onChange={handleSearchChange} />
        ...
        {isHost && (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => handleOpenDialogBulk("kick regular users")}>Kick Regular Users</DropdownMenuItem>
              <DropdownMenuItem onClick={() => handleOpenDialogBulk("kick moderators")}>Kick Moderators</DropdownMenuItem>
              <DropdownMenuItem onClick={() => handleOpenDialogBulk("kick everyone")}>Kick Everyone</DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={() => handleOpenDialogBulk("ban regular users")}>Ban Regular Users</DropdownMenuItem>
              <DropdownMenuItem onClick={() => handleOpenDialogBulk("ban moderators")}>Ban Moderators</DropdownMenuItem>
              <DropdownMenuItem onClick={() => handleOpenDialogBulk("ban everyone")}>Ban Everyone</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        )}
      </div>
      ...
      <div className="max-h-96 overflow-y-auto rounded-lg p-2">
        {filteredMembers.map((member) => (
          <div key={member.id} className="flex items-center justify-between p-3 hover:bg-gray-50 dark:hover:bg-gray-800 rounded-lg">
            <Link href={`/user/${member.username}`} className="flex items-center text-inherit no-underline flex-1">
              ...
              <div className="flex items-center">
                ...
                {Object.keys(data.moderators).includes(member.id) && <ModIcon className="mr-1 h-4 w-4" />}

                <Typography>{member.username}</Typography>
              </div>
            </Link>
            <div className="flex gap-2">
              <Button variant="ghost" size="sm" onClick={() => handleOpenDialog(member, "leave")} className="text-orange-500 hover:text-orange-600">
                <KickIcon className="h-4 w-4" />
              </Button>
              <Button variant="ghost" size="sm" onClick={() => handleOpenDialog(member, "ban")} className="text-red-500 hover:text-red-600">
      ...
      {isHost && <ConfirmationBulkDialog open={uiState.dialogOpenBulk} group={uiState.selectedGroup} onClose={handleCloseDialogBulk} onConfirm={(action) => handleBulkAction(uiState.selectedGroup, action)} />}

      <ConfirmationDialog open={uiState.dialogOpen} user={uiState.selectedUser} action={uiState.selectedGroup.includes("ban") ? "ban" : "kick"} onClose={handleCloseDialog} onConfirm={() => handleAction(uiState.selectedGroup.includes("ban") ? "ban" : "leave", uiState.selectedUser.id)} />

      <BannedUsersDialog open={uiState.bannedDialogOpen} bannedUsers={data.bannedUsers} onClose={handleBannedDialogClose} onConfirm={(action, userId) => handleAction(action, userId)} />
    </div>
  );
}
        ...
        <div className="max-h-96 overflow-y-auto rounded-lg p-2">
          {bannedUsers.map((user) => (
            <div key={user.id} className="flex items-center justify-between p-3 gap-6">
              ...
              <Button variant="ghost" size="sm" onClick={() => onConfirm("unban", user.id)} className="text-blue-500 hover:text-blue-600">
