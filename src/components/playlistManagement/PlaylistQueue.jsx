...
import { getPlaylistData, getBannedSongs, removeSong, banSong, unbanSong } from "@/lib/actions";
...
import { Trash2 as DeleteIcon, Ban as BanIcon, Undo as UndoIcon } from "lucide-react";
...
export default function PlaylistSettings({ playlistId }) {
  ...
  const fetchQueue = async () => {
    try {
      const { data: queueData, error: queueError } = await supabase.from("queue").select("id, title, author, score, url, user_id, added_at, users(id, username)").eq("playlist", playlistId);
              ...
              <TableRow key={song.id}>
                ...
                <TableCell>
                  <div className="flex gap-2">
                    <Button variant="ghost" size="sm" onClick={() => handleDialogOpen("delete", song.id)} className="text-orange-500 hover:text-orange-600">
                      <DeleteIcon className="h-4 w-4" />
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => handleDialogOpen("ban", song.url)} className="text-red-500 hover:text-red-600">
            ...
            {bannedSongs.map((song, index) => (
              <div key={index} className="flex justify-between items-start p-3 border-b last:border-b-0">
                <div className="flex-1">
                  ...
                  <div className="mt-2">
                    <Link href={song.url} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:text-blue-600 text-sm break-all">
                      {song.url}
                    </Link>
                  </div>
                </div>
                <Button variant="ghost" size="sm" onClick={() => handleUnbanSong(song.url)} className="text-blue-500 hover:text-blue-600 ml-4">
