...
export default function Queue({ playlist }) {
      ...
      try {
        ...
        if (searchQuery.trim() !== "") {
          if (searchFilter === "title") {
            ...
            const { data: users, error: userError } = await supabase.from("users").select("id").ilike("username", `%${searchQuery}%`);
  ...
  return (
    <div className="flex flex-col items-center gap-6 w-full px-4 py-6 min-w-[100px]">
      ...
      {/* Songs list */}
      {!loading ? <div className="w-full flex flex-wrap justify-center gap-6">{songs.length === 0 && !loading ? <p className="text-muted-foreground text-center w-full">Playlist is empty</p> : songs.slice((page - 1) * pageSize, page * pageSize).map((song) => <SongCard key={song.id} id={song.id} currentlyPreviewingSongId={currentlyPreviewingSongId} setCurrentlyPreviewingSongId={setCurrentlyPreviewingSongId} />)}</div> : <SkeletonQueue />}
