...
export default function Queue({playlist}) {
    ...
    useEffect(() => {
        async function fetchQueue() {
            ...
            try {
                let queryBuilder = supabase.from("queue").select("*").eq("playlist", playlist);
                ...
                if (searchQuery.trim() !== "") {
                    if (searchFilter === "title") {
                        ...
                        const {
                            data: users,
                            error: userError
                        } = await supabase.from("users").select("id").ilike("username", `%${searchQuery}%`);
    ...
    return (
        <div className="flex flex-col items-center gap-6 w-full px-4 py-6 min-w-[100px]">
            ...
            <div className="w-full max-w-[1100px] flex items-center justify-center p-4 rounded-2xl shadow-md">
                <SearchField onSearchChange={handleSearchChange} onSortOrderChange={handleSortOrderChange}/>
            </div>

            {/* Songs list */}
            {!loading ? <div className="w-full flex flex-wrap justify-center gap-6">{songs.length === 0 && !loading ?
                <p className="text-muted-foreground text-center w-full">Playlist is
                    empty</p> : songs.slice((page - 1) * pageSize, page * pageSize).map((song) => <SongCard
                    key={song.id} id={song.id} currentlyPreviewingSongId={currentlyPreviewingSongId}
                    setCurrentlyPreviewingSongId={setCurrentlyPreviewingSongId}/>)}</div> : <SkeletonQueue/>}
