...
import { Eye, Info, LogOut, MoreVertical, Settings, Star, Trash2, Users } from "lucide-react";
        ...
        const fetchPlaylists = async () => {
            try {
                const {
                    data,
                    error
                } = await supabase.from("users").select("playlists").eq("id", currentUser.id).single();
        ...
        <TooltipProvider>
                                ...
                                <Link href={`/playlist/${playlist.url}`} className="group">
                                    <div className="flex items-center mb-2">
                                        ...
                                        <Typography variant="body1"
                                                    className="font-bold text-blue-400 group-hover:text-purple-400 underline cursor-pointer">
                                            {playlist.name}
                                        </Typography>
                                    </div>
                                </Link>
                            ...
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                    ...
                                    {(currentUser?.id === playlist.host || playlist.moderators.includes(currentUser?.id)) && (
                                        <DropdownMenuItem asChild>
                                            <Link href={`/playlist/${playlist.url}/manage`}
            ...
            <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
                <DialogContent>
                    ...
                    <Typography>Are you sure you want to delete this playlist? This action cannot be
                        undone.</Typography>
                    <DialogFooter>
