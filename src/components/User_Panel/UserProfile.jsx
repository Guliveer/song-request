...
import { getUserSongs, getUserVotes, removeSong, removeVotes, hardBanUser, followUser, unfollowUser, getUserInfo, getCurrentUser, isUserAdmin, isFollowingUser, genUserAvatar, isUserLoggedIn, getJoinedPlaylists, getPlaylistData } from "@/lib/actions";
      ...
      <Container maxWidth="lg" className="mt-8">
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex gap-4">
              {!isSameUser && isLoggedIn && (
                <Button variant="outline" onClick={handleFollowToggle} disabled={followLoading} className="flex items-center gap-2">
                  {userFollowed ? <UserMinus className="h-4 w-4" /> : <UserPlus className="h-4 w-4" />}
                  {followLoading ? "Processing..." : userFollowed ? "Unfollow" : "Follow"}
                </Button>
              )}
              {isAdmin && !isSameUser && (
                <Button variant={userData.ban_status > 0 ? "default" : "destructive"} onClick={handleBanClick}>
                  ...
                  <div className="space-y-4">
                    ...
                    {isAdmin && (
                      <div className="flex justify-between">
                        ...
                        <Badge variant={userData.ban_status > 0 ? "destructive" : "default"}>{userData.ban_status > 0 ? "Banned" : "Active"}</Badge>
                      </div>
                    )}
                    <div className="flex justify-between">
                        ...
                        <tbody>
                          {songs.map((song) => (
                            <tr key={song.id} className="border-b">
                              ...
                              {isAdmin && (
                                <td className="p-4">
                                  <Button variant="outline" size="sm" onClick={() => handleDeleteClick(song.id, "song")} className="text-red-500 border-red-500 hover:bg-red-500/10">
                                    Remove
                                  </Button>
                                </td>
                              )}
                        ...
                        <tbody>
                          {votes.map((vote) => (
                            <tr key={vote.id} className="border-b">
                              ...
                              <td className="p-4">
                                <Badge variant={vote.vote > 0 ? "default" : "destructive"}>{vote.vote > 0 ? "Upvote" : "Downvote"}</Badge>
                              </td>
                              <td className="p-4">{new Date(vote.voted_at).toLocaleString()}</td>
                              {isAdmin && (
                                <td className="p-4">
                                  <Button variant="outline" size="sm" onClick={() => handleDeleteClick(vote.id, "vote")} className="text-red-500 border-red-500 hover:bg-red-500/10">
                                    Remove
                                  </Button>
                                </td>
                              )}
        ...
        <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
          <DialogContent>
            ...
            <Typography>Are you sure you want to remove this {deleteType}? This action cannot be undone.</Typography>
            <DialogFooter>
        ...
        <Dialog open={banDialogOpen} onOpenChange={setBanDialogOpen}>
          <DialogContent>
            ...
            <DialogFooter>
              ...
              <Button variant={userData.ban_status > 0 ? "default" : "destructive"} onClick={handleBanConfirm}>
