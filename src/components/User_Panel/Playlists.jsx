...
export default function Playlists() {
    ...
    const handleMenuClose = () => {
        setAnchorEl(null);
        if (anchorEl == null) { setSelectedPlaylist(null) }
    };
    ...
    return (
        <>
            ...
            <List>
                {playlists.map((playlist, index) => (
                    <Box key={playlist.id}>
                        ...
                        >
                            <Box>
                                <Typography variant="body1" sx={{ fontWeight: "bold", color: "primary.main", display: "inline-flex", alignItems: "center" }}>
            ...
            >
                <MenuItem>
                    <Link href={`/playlist/${selectedPlaylist?.url}`} passHref>
                ...
                {(currentUser?.id !== selectedPlaylist?.host) && (
                    <MenuItem onClick={() => setConfirmDialogOpen(true)} sx={{ color: (theme) => theme.palette.error.main }}>
                        <Box sx={{ display: "flex", alignItems: "center" }}>
                ...
                {(currentUser?.id === selectedPlaylist?.host) && (
                    <MenuItem onClick={() => setDeleteDialogOpen(true)} sx={{ color: (theme) => theme.palette.error.main }}>
                        <Box sx={{ display: "flex", alignItems: "center" }}>
