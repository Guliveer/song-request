...
import { ArrowDownWideNarrow, ArrowUpWideNarrow, Filter, Search } from "lucide-react";
...
export default function SearchField({onSearchChange, onSortOrderChange}) {
    ...
    return (
        <div
            className="flex flex-col sm:flex-row w-full gap-3 max-w-full items-center border rounded-lg bg-card px-5 py-5 shadow-md h-auto sm:h-20 ">
            ...
            <div className="flex w-full sm:w-auto gap-3 mt-2 sm:mt-0 justify-end">
                    ...
                    <PopoverContent>
                        <Command>
                            <CommandList>
                                <CommandGroup>
                                    {FILTERS.map((f) => (
                                        <CommandItem key={f.value} onSelect={() => selectFilter(f.value)}>
                                            {f.label}
                                        </CommandItem>
                                    ))}
                                </CommandGroup>
                ...
                {/* Sort Button Group */}
                <div className="flex">
                    <Button variant="outline" onClick={toggleOrder}
                            className="rounded-none rounded-l-md border-r-0 px-3" aria-label="Toggle sort direction">
                        {sortOrder === "asc" ? <ArrowUpWideNarrow className="w-4 h-4"/> :
                            <ArrowDownWideNarrow className="w-4 h-4"/>}
                    </Button>
                    <Popover open={openSort} onOpenChange={setOpenSort}>
                        <PopoverTrigger asChild>
                            <Button variant="outline" className="rounded-none rounded-r-md border-l-0 px-3">
                                ...
                                <CommandList>
                                    <CommandGroup>
                                        {SORT_OPTIONS.map((opt) => (
                                            <CommandItem key={opt.value} onSelect={() => selectField(opt.value)}>
                                                {opt.label}
                                            </CommandItem>
                                        ))}
                                    </CommandGroup>
